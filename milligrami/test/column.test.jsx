...
describe("Columns", () => {
  ...
  it("Column", () => {
    ...
    assert.equal(column.className, "column");
  });
  it("Column-width", () => {
    const column = document.getElementById("column-width");
    assert.equal(column.tagName.toLowerCase(), "div");
    assert.equal(column.className, "column column-10");
  });
  it("Column-offset", () => {
    const column = document.getElementById("column-offset");
    assert.equal(column.tagName.toLowerCase(), "div");
    assert.equal(column.className, "column column-offset-10");
  });
  it("Column-width-offset", () => {
    const column = document.getElementById("column-width-offset");
    assert.equal(column.tagName.toLowerCase(), "div");
    assert.equal(column.className, "column column-10 column-offset-10");
  });
